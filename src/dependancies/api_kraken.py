...
# TO DO:
# - figure out how append to json files, why is it said to be tricky?
# - add error handling
# - think about rate limits and using sleep function
# - do I need to add the caller ID? user-agent thing
# - currently I have prices as strings, shall I convert them to decimals at the list comprehension stage?
# - Black, Lint and GitActions to automate it
...
data1 = requests.get('https://api.kraken.com/0/public/Depth',params = request_params).json()
...
time_asks = [pd.Timestamp(record[2], unit='s') for record in data1['result']['XXBTZUSD']['asks']]
...
time_bids = [pd.Timestamp(record[2], unit='s') for record in data1['result']['XXBTZUSD']['bids']]


order_book = {'time_asks':time_asks,
